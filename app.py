...
def get_pathway_data(pathway_id: str):
    ...
    for line in data.split('\n'):
        if line.startswith('GENE'):
            ...
            gene_record = False; metabolite_record = False; continue
...
if sel:
    ...
    if any(metabolites_by_freq.values()):
        for freq in sorted(metabolites_by_freq.keys(), reverse=True):
            ...
            # **MODIFIED: Smart Preview Logic**
            if len(df_page) <= 20:
                st.write(f"Showing all {len(df_page)} shared metabolites:")
                display_df = df_page
            else:
                st.write(f"Showing a preview of the top 20 of {len(df_page)} total metabolites:")
                display_df = df_page.head(20)
    ...
    for node, attr in G.nodes(data=True):
        ...
        if attr["type"] == "pathway":
            ...
            color.append("red" if count > 1 else "lightcoral")
        elif attr["type"] == "metabolite":
            count = attr.get("count", 1)
            text.append(f"{node} (in {count} pathways)")
            color.append("gold" if count > 1 else "lightyellow")
