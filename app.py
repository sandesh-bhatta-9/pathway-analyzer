...
def get_pathway_data(pathway_id: str):
    ...
    for line in data.split('\n'):
        if line.startswith('GENE'):
            gene_record = True; metabolite_record = False; line_cont = line[12:].strip()
        elif line.startswith('COMPOUND'):
            metabolite_record = True; gene_record = False; line_cont = line[12:].strip()
        elif (gene_record or metabolite_record) and line.startswith(' '):
            line_cont = line.strip()
        else:
            gene_record, metabolite_record = False, False; continue
        if line_cont:
            parts = line_cont.split()
            if parts:
                identifier = parts[0]
                if gene_record and parts[0].isdigit():
                    genes.add(parts[1])
                elif metabolite_record:
                    metabolites.add(identifier)
    return genes, metabolites

def generate_icon_links(name: str, id_type: str) -> dict:
    """Generates HTML for clickable icons for display."""
    html = {}
    if id_type == 'gene':
        urls = {
        ...
        if b64src:
            html["KEGG"] = (f'<a href="{url}" target="_blank"><img src="{b64src}" width="24" height="24" style="margin:2px; border-radius:4px; border:1px solid #ccc;"></a>')
        else:
            html["KEGG"] = f'<a href="{url}" target="_blank">KEGG</a>'
    return html

def generate_url_links(name: str, id_type: str) -> dict:
    """Generates plain URLs for the CSV download."""
    if id_type == 'gene':
        return {
            "GeneCards_URL": f"https://www.genecards.org/Search/Keyword?queryString={name}",
            "NCBI_URL": f"https://www.ncbi.nlm.nih.gov/gene/?term={name}",
            "ENSEMBL_URL": f"https://useast.ensembl.org/Search/Results?q={name}",
            "GEO_URL": f"https://www.ncbi.nlm.nih.gov/gds/?term={name}",
        }
    elif id_type == 'metabolite':
        return {"KEGG_URL": f"https://www.kegg.jp/entry/{name}"}
    return {}
...
if sel:
    ...
    if any(genes_by_freq.values()):
        for freq in sorted(genes_by_freq.keys(), reverse=True):
            ...
            # Create data for display (with icons)
            display_data = [{'Gene': g, 'Pathways': ', '.join([n for n, gs in p2g.items() if g in gs]), **generate_icon_links(g, 'gene')} for g in gene_list]
            df_display = pd.DataFrame(display_data).sort_values(by="Gene")
            
            # Create data for download (with URLs)
            download_data = [{'Gene': g, 'Pathways': ', '.join([n for n, gs in p2g.items() if g in gs]), **generate_url_links(g, 'gene')} for g in gene_list]
            df_download = pd.DataFrame(download_data).sort_values(by="Gene")

            # Smart Preview Logic
            if len(df_display) <= 20:
                st.write(f"Showing all {len(df_display)} shared genes:")
            else:
                st.write(f"Showing a preview of the top 20 of {len(df_display)} total genes:")
            
            display_cols = ['Gene','Pathways','Gene Cards','NCBI','ENSEMBL','GEO']
            st.write(df_display[display_cols].head(20).to_html(escape=False,index=False), unsafe_allow_html=True)
            
            csv_string = df_download.to_csv(index=False).encode('utf-8')
            st.download_button(label=f"Download Full List ({len(df_download)} genes)", data=csv_string, file_name=f"shared_genes_{freq}_pathways.csv", mime="text/csv", key=f"dl_gene_{freq}")
            st.markdown("---")
    else:
    ...
    if any(metabolites_by_freq.values()):
        for freq in sorted(metabolites_by_freq.keys(), reverse=True):
            ...
            display_data = [{'Metabolite': m, 'Pathways': ', '.join([n for n, ms in p2m.items() if m in ms]), **generate_icon_links(m, 'metabolite')} for m in metabolite_list]
            df_display = pd.DataFrame(display_data).sort_values(by="Metabolite")
            
            download_data = [{'Metabolite': m, 'Pathways': ', '.join([n for n, ms in p2m.items() if m in ms]), **generate_url_links(m, 'metabolite')} for m in metabolite_list]
            df_download = pd.DataFrame(download_data).sort_values(by="Metabolite")

            if len(df_display) <= 20:
                st.write(f"Showing all {len(df_display)} shared metabolites:")
            else:
                st.write(f"Showing a preview of the top 20 of {len(df_display)} total metabolites:")
            
            cols = ['Metabolite','Pathways','KEGG']
            st.write(df_display[cols].head(20).to_html(escape=False,index=False), unsafe_allow_html=True)
            
            csv_string = df_download.to_csv(index=False).encode('utf-8')
            st.download_button(label=f"Download Full List ({len(df_download)} metabolites)", data=csv_string, file_name=f"shared_metabolites_{freq}_pathways.csv", mime="text/csv", key=f"dl_metabolite_{freq}")
            st.markdown("---")
    else:
        st.write("⚠️ No shared metabolites found.")

    # --- Network Visualization ---
    st.header("🕸️ Pathway Network (Genes & Metabolites)")
    G = nx.Graph()
    for pathway in p2g: G.add_node(pathway, type="pathway")
    for gene, cnt in gene_counts.items(): G.add_node(gene, type="gene", count=cnt)
    for met, cnt in metabolite_counts.items(): G.add_node(met, type="metabolite", count=cnt)
    for pathway, genes in p2g.items():
        for gene in genes: G.add_edge(pathway, gene)
    for pathway, metabolites in p2m.items():
        for met in metabolites: G.add_edge(pathway, met)
    ...
    fig = go.Figure(data=[edge_trace, node_trace], layout=go.Layout(title="Gene & Metabolite Network", xaxis=dict(showgrid=False, zeroline=False, showticklabels=False), yaxis=dict(showgrid=False, zeroline=False, showticklabels=False), margin=dict(b=20, l=5, r=5, t=40), hovermode='closest'))
